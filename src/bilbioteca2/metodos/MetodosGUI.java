...
public class MetodosGUI {
    ...
    public static DefaultTableModel cargarPrestamos(){
        DefaultTableModel tabla = new DefaultTableModel();
        ...
        tabla.addColumn("FECHA DEVOLUCION");
        tabla.addColumn("AUMENTO");
        tabla.addColumn("DEVUELTOS");
        ArrayList <Prestamos> prestamos = ConexionBD.mostrarPrestamos();
        Iterator it = prestamos.iterator();
        while(it.hasNext()){
                Prestamos prestamo = (Prestamos) it.next();
                String[] fila = new String[6];
                fila[0] = ConexionBD.nombresUsuarios(prestamo.getCodUsuario());
                fila[1] = prestamo.getLibro();
                fila[2] = prestamo.getFechaPrestamo();
                fila[3] = prestamo.getFechaDevolucion();
                if (prestamo.isDevuelto()){
                    fila[4] = String.valueOf("Devuelto");
                }else{
                    fila[4] = String.valueOf("No devuelto");
                }
                fila[5] = String.valueOf(prestamo.getAumento())+" aumento/s";
                tabla.addRow(fila);
            }
            return tabla;
    }
